/datum/trader  // Credit to Urist McStation for their trading code.
	var/name = "Generic Trader"                            //Designates the name of the Trader.
	var/origin = "Somewhere"                                   //Shows the origin of the Trader, e.g: Earth.
	//var/list/possible_origins                                //Randomly designates possible list of names. [REMOVED FOR NOW]
	var/reputation = 0                                         //The current reputation of them to us.
	var/trade_flags = TRADER_MONEY                              //Flags
	//var/name_language                                         //If this is set to a language name this will generate a name from the language [REMOVED]
	var/icon/portrait                                           // Displays a portrait.
	...
	insult_good			When the player insults a merchant while they are on good reputation
	insult_bad			When a player insults a merchatn when they are not on good reputation
	...
	*/
	...
	var/insult_drop = 5                                         //How far reputation drops on insult
	var/compliment_increase = 5                                 //How far compliments increase reputation
	var/refuse_comms = 0                                        //Whether they refuse further communication

	var/mob_transfer_message = "You are suddenly transported to ORIGIN." //What message gets sent to mobs that get sold.

/datum/trader/New()
	..()
	//if(name_language)						THIS DOESNT WORK AT ALL, TRIED TO DEFINE IN _GLOBAL_VARS BUT IT WONT PLAY BALL
		//if(name_language == TRADER_DEFAULT_NAME)
			//name = capitalize(pick(GLOB.first_female + GLOB.first_male)) + " " + capitalize(pick(GLOB.last_names))
		//else
			//var/datum/language/L = all_languages[name_language]
			//if(L)
				//name = new_species.name(pick(MALE,FEMALE))
...
/datum/trader/proc/get_item_value(var/trading_num)
	if(!trading_items[trading_items[trading_num]])
		...
		var/value = get_value(type)		//Undefined Proc, get_value error
...
/datum/trader/proc/offer_items_for_trade(var/list/offers, var/num, var/turf/location)
	...
	for(var/item in offers)
		...
		offer_worth += get_value(offer) * (is_wanted > 1 ? want_multiplier : 1)		//Undefined Proc, get_value error
	if(!offer_worth)
		return TRADER_NOT_ENOUGH
	...
	if(percent > max(0.9,0.9-reputation/100))
...
/datum/trader/proc/can_hail()
	if(!refuse_comms && prob(-reputation))
		refuse_comms = 1
	return !refuse_comms

/datum/trader/proc/insult()
	reputation -= rand(insult_drop, insult_drop * 2)
	if(prob(-reputation/10))
		refuse_comms = 1
	if(reputation > 50)
...
/datum/trader/proc/compliment()
	...
	if(prob(100-reputation))
		reputation += rand(compliment_increase, compliment_increase * 2)
...
/datum/trader/proc/trade(var/list/offers, var/num, var/turf/location)
	...
	reputation += rand(compliment_increase,compliment_increase*3) //Traders like it when you trade with them
...
/datum/trader/proc/sell_items(var/list/offers)
	...
	for(var/offer in offers)
		if((trade_flags & TRADER_WANTED_ONLY) && is_type_in_list(offer,wanted_items))
			...
			return TRADER_FOUND_UNWANTED
		. += get_value(offer) * mult	//Undefined Proc, get_value error
